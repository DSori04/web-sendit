        ...
        <nav className="bg-gray1 w-full h-16 flex lg:justify-between z-40 justify-center lg:pl-24 select-none fixed">
            ...
            <div id="menu" className="lg:flex hidden flex-row w-9/12 text-center align-middle justify-around pl-16 select-none font-main text-gray2">
                ...
                <div id="imgs" className="flex flex-row justify-around"  >
                    <Link to="/tracking" onMouseEnter={() => setTracking(true)} onMouseLeave={() => setTracking(false)}><img src={Tracking} className="text-center w-16 h-12 pt-2 pl-2 pb-1 hover:scale-110" alt="tracking-icon"></img></Link>
                    <Link to="/new" onMouseEnter={() => setNew(true)} onMouseLeave={() => setNew(false)}><img src={Box} className="text-center h-10 pt-2 pl-2 hover:scale-110" alt="box-icon"></img></Link>
                    <Link to="/prices" onMouseEnter={() => setPrice(true)} onMouseLeave={() => setPrice(false)}><img src={Prices} className="text-center h-12 w-16 hover:scale-110" alt="prices-icon"></img></Link>
                </div>
                <div id="links" className={`flex flex-row justify-around -mt-2`}>
                    <Link to="/tracking" className={`text-center ${tracking ? 'text-gray2' : 'text-gray1'}`}>Tracking</Link>
                    <Link to="/new" className={`text-center ${newMenu ? 'text-gray2' : 'text-gray1'}`}>Nuevo</Link>
                    <Link to="/prices" className={`text-center ${price ? 'text-gray2' : 'text-gray1'}`}>Precios</Link>
                </div>
